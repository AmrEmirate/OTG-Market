...
import OtgTiJasaModal from '../components/modals/OtgTiJasaModal';
import OtgTiPremiumModal from '../components/modals/OtgTiPremiumModal';
...
const FeeTransaksiView = () => {
  ...
  const isOtgTiJasaModalOpen = path === '/fee/otg-ti-jasa';
  const isOtgTiPremiumModalOpen = path === '/fee/otg-ti-premium';
  ...
  return (
    <section className="w-full flex flex-col gap-4 animate-fade-in">
      ...
      {feeLinks.map((link, index) => {
        ...
        if (link.title === 'Jasa OTG Market TI') slug = 'otg-ti-jasa';
        if (link.title === 'Jasa Premium Otg Market TI') slug = 'otg-ti-premium';
      ...
      <OtgTiJasaModal isOpen={isOtgTiJasaModalOpen} onClose={handleClose} />
      <OtgTiPremiumModal isOpen={isOtgTiPremiumModalOpen} onClose={handleClose} />
    </section>
  );
};
